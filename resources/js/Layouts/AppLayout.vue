<template>
    <div @keydown.window.escape="sidebar = false" class="h-screen overflow-hidden flex bg-gray-100">
        ...
        <div v-show="sidebar" class="fixed inset-0 z-40 flex md:hidden" ref="dialog" aria-modal="true">
            <transition enter-class="transition-opacity ease-linear duration-300" enter-active-class="opacity-0"
                enter-to-class="opacity-100" leave-class="transition-opacity ease-linear duration-300"
                leave-active-class="opacity-100" leave-to-class="opacity-0">
                <div v-show="sidebar" class="fixed inset-0 bg-gray-600 bg-opacity-75" @click="sidebar = false"
                    aria-hidden="true">
                </div>
            </transition>
            <transition enter-class="transition ease-in-out duration-300 transform" enter-active-class="-translate-x-full"
                enter-to-class="translate-x-0" leave-class="transition ease-in-out duration-300 transform"
                ...
                <div v-show="sidebar" class="relative max-w-xs w-full bg-primary pt-5 pb-4 flex-1 flex flex-col">
                    <transition enter-class="ease-in-out duration-300" enter-active-class="opacity-0"
                        enter-to-class="opacity-100" leave-class="ease-in-out duration-300" leave-active-class="opacity-100"
        ...
        <div class="flex-1 w-full mx-auto w-0 flex flex-col md:px-0">
            <div class="relative z-20 flex-shrink-0 flex h-16 bg-white sm:border-b border-gray-200 lg:border-none">
                ...
                <div class="flex-1 max-w-5xl mx-auto border-b border-gray-200 flex justify-between px-4 md:px-0">
                    ...
                    <div class="ltr:ml-4 rtl:mr-4 flex items-center md:ltr:ml-6 md:rtl:mr-6">
                        <!--Rewards Badge-->
                        <rewards-badge :text-color="'text-primary'"
                            :points="$page.props.user.wallet_balance"></rewards-badge>
                                ...
                                <template #trigger>
                                    <button v-if="$page.props.jetstream.managesProfilePhotos"
                                        ...
                                        <img class="h-8 w-8 rounded-full object-cover"
                                            :src="$page.props.user.profile_photo_url" :alt="$page.props.user.first_name" />
                                    </button>
            ...
            <perfect-scrollbar class="flex-1 relative h-full z-0 overflow-hidden focus:outline-none" ref="scroll"
