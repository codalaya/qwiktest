            ...
            <div class="card">
                <div class="card-body">
                    <vue-good-table mode="remote" @on-page-change="onPageChange" @on-column-filter="onColumnFilter"
                        ...
                        :totalRows="payments.meta.pagination.total" :rows="payments.data" :rtl="$page.props.rtl">
                        <template slot="table-row" slot-scope="props">
                            <!-- Code Column -->
                            <div v-if="props.column.field === 'code'">
                                <Tag v-clipboard:copy="props.row.code" :value="props.row.code"
                                    v-clipboard:success="handleCopyStatus" icon="pi pi-copy"
                            ...
                            <div v-else-if="props.column.field === 'actions'">
                                <Button v-if="props.row.status === 'pending' && props.row.method === 'Bank'"
                                ...
                                <actions-dropdown>
                                    <template #actions>
                                        <button @click="showDetails = true; currentId = props.row.id;"
                                            class="action-item">{{ __('Details') }}</button>
                                        <button @click="deletePayment(props.row.id)" class="action-item">{{ __('Delete')
                                        }}</button>
                                    </template>
                                </actions-dropdown>
                            </div>
                    ...
                    <Sidebar position="right" :visible.sync="showDetails" class="p-sidebar-md">
                        <PaymentDetails :errors="errors" :status-types="paymentStatuses" @close="showDetails = false"
                            :payment-id="currentId" :title="__('Payment') + ' ' + __('Details')" />
                    </Sidebar>
                </div>
            </div>
        </div>
