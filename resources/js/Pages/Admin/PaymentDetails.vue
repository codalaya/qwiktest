                ...
                <dl>
                    ...
                    <div v-if="!editFlag" class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2 uppercase">
                            {{ payment.status }} <span class="ml-2 text-xs text-blue-500 cursor-pointer underline"
                                @click="editFlag = true">{{ __('Edit') }}</span>
                        </dd>
                    </div>
                    <div v-if="editFlag" class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">
                            ...
                            <form @submit.prevent="update">
                                ...
                                <div class="col-span-6 sm:col-span-4">
                                    <arc-label for="status">
                                        {{ __('Status') }} <span class="ml-2 text-xs text-blue-500 cursor-pointer underline"
                                            @click="editFlag = false">{{ __('Close') }}</span>
                                    </arc-label>
                                    <v-select id="status" v-model="form.status" :options="statusTypes"
                                        :reduce="t => t.value" label="text" :dir="$page.props.rtl ? 'rtl' : 'ltr'" />
                                    <arc-input-error v-if="errors !== undefined && errors.status" :message="errors.status"
                                        class="mt-2" />
                                </div>
                                <arc-button :class="{ 'opacity-25': form.processing }" class="mt-1"
                            ...
                            <ul role="list" class="border border-gray-200 rounded-md divide-y divide-gray-200">
                                <li class="pl-3 pr-4 py-3 flex items-center justify-between text-sm">
                                    ...
                                    <div class="ml-4 flex-shrink-0">
                                        <a :href="route('download_invoice', { id: payment.payment_id })" target="_blank"
