    ...
    <div class="overflow-y-auto h-screen px-2">
        ...
        <form v-else class="my-6 w-11/12 mx-auto xl:w-full xl:mx-0" @submit.prevent="submitForm">
            <div class="w-full flex flex-col mb-6">
                <label class="pb-2 text-sm font-semibold text-gray-800">{{ __('Schedule Type') }}<span
                        class="ml-1 text-red-400">*</span></label>
                <div class="flex flex-col sm:flex-row gap-4">
                    <div class="p-field-radiobutton" v-for="scheduleType in scheduleTypes">
                        <RadioButton :id="'schedule_type' + scheduleType.code" name="schedule_type"
                            :value="scheduleType.code" v-model="form.schedule_type" :disabled="editFlag" />
                        <label class="text-sm text-gray-800" :for="'schedule_type' + scheduleType.code">{{ scheduleType.name
                        }}</label>
                    </div>
                </div>
            </div>
            <div class="flex flex-col sm:flex-row gap-2">
                <div class="w-full flex flex-col mb-6">
                    ...
                    <VueCtkDateTimePicker id="start_date" :label="'Select Start Date'" v-model="form.start_date"
                        :only-date="true" :formatted="'MMMM Do YYYY'" :format="'YYYY-MM-DD'" :no-label="true"
                        :min-date="minDate" :no-header="true" :class="[errors.start_date ? 'p-invalid' : '']"
                ...
                <div class="w-full flex flex-col mb-6">
                    ...
                    <VueCtkDateTimePicker id="start_time" :label="'Select Start Time'" v-model="form.start_time"
                        :only-time="true" :formatted="'HH:mm'" :format="'HH:mm:ss'" :no-label="true" :no-header="true"
                        :minute-interval="1" :class="[errors.start_time ? 'p-invalid' : '']" :disabled="disableFlag" />
            ...
            <div v-if="form.schedule_type === 'flexible'" class="flex flex-col sm:flex-row gap-2">
                <div class="w-full flex flex-col mb-6">
                    ...
                    <VueCtkDateTimePicker id="end_date" :label="'Select End Date'" v-model="form.end_date" :only-date="true"
                        :formatted="'MMMM Do YYYY'" :format="'YYYY-MM-DD'" :no-label="true" :min-date="minDate"
                        :no-header="true" :class="[errors.end_date ? 'p-invalid' : '']" :disabled="disableFlag" />
                ...
                <div class="w-full flex flex-col mb-6">
                    ...
                    <VueCtkDateTimePicker id="end_time" :label="'Select End Time'" v-model="form.end_time" :only-time="true"
                        :formatted="'HH:mm'" :format="'HH:mm:ss'" :no-label="true" :no-header="true" :minute-interval="1"
                        :class="[errors.end_time ? 'p-invalid' : '']" :disabled="disableFlag" />
            ...
            <div v-if="form.schedule_type === 'fixed'" class="w-full flex flex-col mb-6">
                <label for="grace_period" class="pb-2 font-semibold text-gray-800">{{ __('Grace Period to Join') }} ({{
                    __('Minutes') }})</label>
                <InputNumber id="grace_period" v-model="form.grace_period" placeholder="Enter Grace Period"
                    aria-describedby="grace_period-help" :class="[errors.grace_period ? 'p-invalid' : '']"
                    :disabled="disableFlag" />
            ...
            <div class="w-full flex flex-col mb-6">
                ...
                <v-select multiple id="users" v-model="form.user_groups" :options="userGroups" :reduce="group => group.id"
                    label="name" :disabled="disableFlag" />
            ...
            <div v-if="editFlag" class="w-full flex flex-col mb-6">
                ...
                <Dropdown id="status" v-model="form.status" :options="quizStatus" optionLabel="name" optionValue="code"
                    dataKey="code" placeholder="Select Status" :disabled="disableFlag" />
                <small id="status-help" v-if="errors.status" class="p-invalid">{{ errors.status }}</small>
            </div>
            <small id="expire-note" v-if="disableFlag" class="text-sm text-red-400">{{ __('schedule_update_message')
            }}</small>
