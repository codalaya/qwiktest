...
class QuizCrudController extends Controller
{
    ...
    public function index(QuizFilters $filters)
    {
        return Inertia::render('Admin/Quizzes', [
            'quizzes' => function () use ($filters) {
                return fractal(
                    Quiz::filter($filters)->with(['subCategory:id,name', 'quizType:id,name'])
    ...
    public function create()
    {
        return Inertia::render('Admin/Quiz/Details', [
            ...
            'initialSubCategories' => fractal(
                SubCategory::select(['id', 'name', 'category_id'])->latest()->take(10)->get(),
                new SubCategorySearchTransformer()
            )->toArray()['data']
        ]);
    }
